...
namespace OpenWifi {
	...
	class HistoryDB : public ORM::DB<HistoryRecordTuple, FMSObjects::RevisionHistoryEntry> {
	  public:
		...
		bool AddHistory(const std::string &SerialNumber, const std::string &DeviceType,
		...
		bool GetUnknownDeviceFirmwares(uint64_t offset, uint64_t limit,
									   std::vector<FMSObjects::DeviceCurrentInfo> &Devices);
		bool GetDeviceFirmwares(uint64_t offset, uint64_t limit,
								std::vector<FMSObjects::DeviceCurrentInfo> &Devices);

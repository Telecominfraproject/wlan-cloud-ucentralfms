...
namespace OpenWifi {
	...
	static ORM::FieldVec FirmwaresDB_Fields{ORM::Field{"id", 36, true},
	...
	bool FirmwaresDB::DeleteFirmware(std::string &Id) { return DeleteRecord("id", Id); }
	...
	bool FirmwaresDB::GetFirmwareByName(const std::string &Release, const std::string &DeviceType,
	...
	bool FirmwaresDB::GetFirmwareByRevision(const std::string &Revision,
	...
	bool FirmwaresDB::GetFirmwares(uint64_t From, uint64_t HowMany, const std::string &Compatible,
	...
	bool FirmwaresDB::ComputeFirmwareAge(const std::string &DeviceType, const std::string &Revision,
		...
		try {
			...
			if (LatestFirmwareCache()->FindLatestFirmware(DeviceType, LE)) {
				...
				if (GetFirmware(LE.Id, LatestFirmware)) {
					...
					AgeDetails.age = CurrentFirmwareExists
										 ? (LatestFirmware.imageDate - CurrentFirmware.imageDate)
										 : 0;
					// std::cout << "Revision: '" << Revision << "' vs '" << LatestFirmware.revision
					// << "'" << std::endl; if (AgeDetails.latest)
...
template <>
void ORM::DB<OpenWifi::FirmwaresRecordTuple, OpenWifi::FMSObjects::Firmware>::Convert(
	const OpenWifi::FirmwaresRecordTuple &T, OpenWifi::FMSObjects::Firmware &F) {
	F.id = T.get<0>();
	F.release = T.get<1>();
...
template <>
void ORM::DB<OpenWifi::FirmwaresRecordTuple, OpenWifi::FMSObjects::Firmware>::Convert(
	const OpenWifi::FMSObjects::Firmware &F, OpenWifi::FirmwaresRecordTuple &T) {
	T.set<0>(F.id);
	T.set<1>(F.release);

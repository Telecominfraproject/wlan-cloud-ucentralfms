...
namespace OpenWifi {
	...
	static ORM::FieldVec HistoryDB_Fields{ORM::Field{"id", 36, true},
	...
	HistoryDB::HistoryDB(OpenWifi::DBType T, Poco::Data::SessionPool &P, Poco::Logger &L)
	...
	bool HistoryDB::AddHistory(const std::string &SerialNumber, const std::string &DeviceType,
							   const std::string &PreviousRevision, const std::string &NewVersion) {

		FMSObjects::RevisionHistoryEntry History{.id = MicroServiceCreateUUID(),
	...
	bool HistoryDB::DeleteHistory([[maybe_unused]] const std::string &SerialNumber,
								  const std::string &Id) {
	...
	bool HistoryDB::GetUnknownDeviceFirmwares(uint64_t offset, uint64_t limit,
		...
		try {
			...
			auto sql_query = fmt::format(
				"select max(upgraded), serialnumber from history where revisionid='unknown' group "
				"by serialnumber order by serialnumber offset={} limit={};)",
				offset, limit);
			Poco::Data::Statement sql_statement(sql_session);

			sql_statement << sql_query, Poco::Data::Keywords::into(RecList),
				Poco::Data::Keywords::now;

			for (const auto &record : RecList) {
				FMSObjects::DeviceCurrentInfo entry{.serialNumber = record.get<1>(),
													.revision = "unknown",
	...
	bool HistoryDB::GetDeviceFirmwares(uint64_t offset, uint64_t limit,
		...
		try {
			Poco::Data::Session sql_session = Pool_.get();
			auto sql_query = fmt::format(
				"select max(upgraded), serialNumber, toRelease from history where "
				"revisionId!='unknown' group by serialNumber, toRelease order by serialNumber, "
				"toRelease offset={} limit={};)",
				offset, limit);
			Poco::Data::Statement sql_statement(sql_session);
...
template <>
void ORM::DB<OpenWifi::HistoryRecordTuple, OpenWifi::FMSObjects::RevisionHistoryEntry>::Convert(
	const OpenWifi::HistoryRecordTuple &T, OpenWifi::FMSObjects::RevisionHistoryEntry &F) {
	F.id = T.get<0>();
...
template <>
void ORM::DB<OpenWifi::HistoryRecordTuple, OpenWifi::FMSObjects::RevisionHistoryEntry>::Convert(
	const OpenWifi::FMSObjects::RevisionHistoryEntry &F, OpenWifi::HistoryRecordTuple &T) {
	T.set<0>(F.id);

...
namespace OpenWifi {
	...
	static ORM::FieldVec DevicesDB_Fields{ORM::Field{"serialNumber", 36, true},
	...
	DevicesDB::DevicesDB(OpenWifi::DBType T, Poco::Data::SessionPool &P, Poco::Logger &L)
		: DB(T, "devices", DevicesDB_Fields, {}, P, L, "fdi") {}

	bool DevicesDB::SetDeviceRevision(std::string &SerialNumber, std::string &Revision,
									  std::string &DeviceType, std::string &EndPoint) {
		FMSObjects::DeviceConnectionInformation D;
		if (GetRecord("serialNumber", SerialNumber, D)) {
			if (D.revision != Revision) {
				StorageService()->HistoryDB().AddHistory(SerialNumber, DeviceType, D.revision,
														 Revision);
			}
			D.revision = Revision;
			...
			FMSObjects::DeviceConnectionInformation DI{.serialNumber = SerialNumber,
													   .revision = Revision,
	...
	bool DevicesDB::GetDevices(uint64_t From, uint64_t HowMany,
	...
	bool DevicesDB::GetDevice(std::string &SerialNumber,
							  FMSObjects::DeviceConnectionInformation &Device) {
...
template <>
void ORM::DB<OpenWifi::DevicesRecordTuple, OpenWifi::FMSObjects::DeviceConnectionInformation>::
	Convert(const OpenWifi::DevicesRecordTuple &T,
			OpenWifi::FMSObjects::DeviceConnectionInformation &F) {
...
template <>
void ORM::DB<OpenWifi::DevicesRecordTuple, OpenWifi::FMSObjects::DeviceConnectionInformation>::
	Convert(const OpenWifi::FMSObjects::DeviceConnectionInformation &F,
			OpenWifi::DevicesRecordTuple &T) {
